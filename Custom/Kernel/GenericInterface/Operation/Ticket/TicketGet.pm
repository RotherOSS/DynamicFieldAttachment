...
# --
# $origin: otobo - 954d3bf84cfcb2666f5d9715f14f4ca2d921621c - Kernel/GenericInterface/Operation/Ticket/TicketGet.pm
# --
...
#                };
                ...
                my ($DynamicFieldConfig) = grep { $_->{Name} eq $DynamicFieldName } @{$TicketDynamicFields};

                if (
                    $DynamicFieldConfig
                    && $DynamicFieldConfig->{FieldType} eq 'Attachment'
                    && IsArrayRefWithData( $TicketEntryRaw{$Attribute} )
                    )
                {
                    my @Value;

                    # This dynamic field type is Attachment, get content.
                    ATTACHMENT:
                    for my $Attachment ( @{ $TicketEntryRaw{$Attribute} } ) {
                        ...
                        push @Value, {
                            ...
                            Content     => encode_base64( $Data->{Content} ),
                            ContentType => $Data->{ContentType},
                            Filename    => $Data->{Filename},
                            FilesizeRaw => $Data->{Filesize},
                        };
                    }

                    $TicketEntryRaw{$Attribute} = \@Value;
                }
