...
sub _AddAction {
    ...
    for my $ConfigParam (
        qw(ObjectType ObjectTypeName FieldType FieldTypeName ValidID NumberOfFiles MaximumFileSize Namespace)
        )
    {
    ...
    if ( !IsPositiveInteger( $GetParam{NumberOfFiles} ) ) {
        $Errors{NumberOfFilesServerError}        = 'ServerError';
        $Errors{NumberOfFilesServerErrorMessage} = Translatable(
            'The maximum of attachments for this dynamic field of type attachment must be a positive integer.'
        );
    }

    # check if MaximumFileSize is numeric
    if ( !IsPositiveInteger( $GetParam{MaximumFileSize} ) ) {
        $Errors{MaximumFileSizeServerError}        = 'ServerError';
        $Errors{MaximumFileSizeServerErrorMessage} = Translatable(
            'The maximum attachment size for this dynamic field of type attachment must be a positive integer.'
        );
    }
